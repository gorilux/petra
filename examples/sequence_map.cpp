...
int main(int argc, char** argv) {
  ...
  if (static_cast<std::size_t>(argc) - 1 > sequence_size) {
    std::cout << "Sorry, we can only take " << sequence_size << " integers from the command line.\n";
    return 255;
  }

  for (int i = 1; i < argc; ++i) {
    test[i-1] = std::stoul(argv[i]);
  }

  // TODO There seems to be a weird interaction with the size and the upper bound here? I have no idea
